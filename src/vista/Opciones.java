...
public class Opciones extends JPanel {
	...
	public Opciones(JFrame ventana) {
		...
		textos_generales.setFont(((Ventana) ventana).getFuenteGeneral());
		textos_generales.setBounds(50, 50, 120, 20);
		
		JCheckBox negrita_general = new JCheckBox("Activar/Desactivar Negrita");
		negrita_general.setFont(((Ventana) ventana).getFuenteGeneral());
		...
		tamanos_general.setFont(((Ventana) ventana).getFuenteGeneral());
		...
		fuentes_general.setFont(((Ventana) ventana).getFuenteGeneral());
		fuentes_general.setBounds(590, 50, 200, 20);
		int index_generales = 0;
		for (int i = 0; i < FUENTES.length; i++) {
			if(FUENTES[i].equalsIgnoreCase(((Ventana) ventana).getFuenteGeneral().getName())){
				index_generales = i;
			}
		}
		...
		textos_tablas.setFont(((Ventana) ventana).getFuenteTablas());
		textos_tablas.setBounds(50, 150, 120, 20);
		
		JCheckBox negrita_tablas = new JCheckBox("Activar/Desactivar Negrita");
		negrita_tablas.setFont(((Ventana) ventana).getFuenteTablas());
		...
		tamanos_tablas.setFont(((Ventana) ventana).getFuenteTablas());
		...
		fuentes_tablas.setFont(((Ventana) ventana).getFuenteTablas());
		fuentes_tablas.setBounds(590, 150, 200, 20);
		int index_tablas = 0;
		for (int i = 0; i < FUENTES.length; i++) {
			if(FUENTES[i].equalsIgnoreCase(((Ventana) ventana).getFuenteTablas().getName())){
				index_tablas = i;
			}
		}
		...
		textos_areas.setFont(((Ventana) ventana).getFuenteAreas());
		...
		int index_areas = 0;
		for (int i = 0; i < FUENTES.length; i++) {
			if(FUENTES[i].equalsIgnoreCase(((Ventana) ventana).getFuenteAreas().getName())){
				index_areas = i;
			}
		}
		...
		aplicar.setFont(((Ventana) ventana).getFuenteGeneral());
		...
		aplicar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				((Ventana) ventana).setFuenteGeneral(new Font(fuentes_general.getSelectedItem().toString(), ((negrita_general.isSelected()) ? Font.BOLD : Font.PLAIN), Integer.parseInt(tamanos_general.getSelectedItem().toString())));
				((Ventana) ventana).setFuenteTablas(new Font(fuentes_tablas.getSelectedItem().toString(), ((negrita_tablas.isSelected()) ? Font.BOLD : Font.PLAIN), Integer.parseInt(tamanos_tablas.getSelectedItem().toString())));
				((Ventana) ventana).setFuenteAreas(new Font(fuentes_areas.getSelectedItem().toString(), ((negrita_areas.isSelected()) ? Font.BOLD : Font.PLAIN), Integer.parseInt(tamanos_areas.getSelectedItem().toString())));
				
				((Ventana) ventana).llamarOpciones();
			}
		});
